#include "Checkered.h"
#include "DLLInfo.h"
Checkered::Checkered(const Color* bgA, const  Color* bgB, const  Color* rect, int bgMOD, int rectA, int screenX, int screenY)noexcept :child(rect, rectA, screenX, screenY) {
	A = Color(bgA);
	B = Color(bgB);
	mod = bgMOD * 2 * scaling;
}
