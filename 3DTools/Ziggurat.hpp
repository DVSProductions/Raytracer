...
namespace cxx {
	namespace ziggurat_detail {
		...
		inline std::uint64_t generate_bits(URNG& random) {
			constexpr std::uint64_t mask = (static_cast<uint64_t>(1) << N) - 1;
		...
		template<typename T>
		inline T gaussian(T x) noexcept {
	...
	public:
		...
		struct param_type {
			...
			// values.
			explicit param_type(result_type mean, result_type stddev = 1)noexcept
			...
			inline result_type stddev() const noexcept {
				return stddev_;
			}
		...
		// stddev.
		explicit ziggurat_normal_distribution(result_type mean, result_type stddev = 1) noexcept
			: param_{ mean, stddev } {}
	...
	// parameters.
	template<typename T>
	bool operator==(ziggurat_normal_distribution<T> const& d1,ziggurat_normal_distribution<T> const& d2) {
	...
	template<typename Char, typename Tr, typename T>
	std::basic_istream<Char, Tr>& operator>>(std::basic_istream<Char, Tr>& is, ziggurat_normal_distribution<T>& dist) {
