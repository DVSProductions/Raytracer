...
int main()
{
	cout << "Hello from LinuxDistro!" << endl;
	printf("loading glass..")	;
	...
	height = 2160;
	cam->init();
	cout << "." << endl << "Setting Scene..";
	std::this_thread::sleep_for(std::chrono::milliseconds(100));
	cam->setScene(playground);
	cout << "." << endl << "Setting Sample Quality..";
	std::this_thread::sleep_for(std::chrono::milliseconds(100));
	Raytracer::setsampleQuality(5);
	cout << "." << endl << "Setting ThreadCount..";
	std::this_thread::sleep_for(std::chrono::milliseconds(100));
	Raytracer::setFullThreadCount();
	cout << "." << endl << "Starting render..";	
	std::this_thread::sleep_for(std::chrono::milliseconds(100));
	Raytracer::RenderWorker(sceneOutputFile);
	cout << "." << endl << "Rendering..."<<endl;
	std::this_thread::sleep_for(std::chrono::milliseconds(100));
	int lastlength = 0;
	while (width != progress) {
		auto s = std::to_string((double)progress * 100 / (double)width);
		printf("\r");
		for (int n = -1; n != lastlength;n++)printf(" ");
		lastlength = s.length();
		cout<< "\r" << s << "%";
		cout.flush();
